...
5. Audio leadership (client-side, `web/src/audioLeader.js`): exactly one non-observer tab per origin plays interrupt/readback audio — Web-Locks leader election with automatic failover when the leading tab closes. Without it, N connected tabs each play the same clip a beat apart (perceived as echo even on earphones; the tell is repeated `GET /tts/<id>.mp3` lines). Non-leader tabs render banner/glow only and never arm the floor machine.
...
**Tools (10) — names, params, result shapes (as shipped):** `draft_add(text)` → `{ok}` · `draft_wrap_up()` → `{ok, markdown}` · `draft_read_back()` → `{ok, text}` · `draft_ignore_objection()` → `{ok, dismissed_id}` · `draft_new_document()` → `{ok, slug}` · `draft_get_document()` → the snapshot `{title, blocks, objections, slug, has_openai_key}` · `draft_export()` → `{ok, markdown}` · `draft_share()` → `{ok, url}` · `draft_list()` → `{ok, configured, drafts:[{index, slug, title, status, updatedAt, blocks, hasFinal}], current_slug}` (indexed newest-first, empty rows filtered; `configured:false` + hint without CONVEX_URL) · `draft_open(query)` → `{ok, slug, title, blocks}` on success or `{ok:false, error, candidates:[{index,title}]}` (query matched as exact slug, list index — digits or spoken words like "second"/"latest" — then title-word subset, then any-overlap, newest first; loads the stored draft into the live session via `session.load_snapshot`, ADOPTS its slug so edits continue in the same Convex row, reopens the row live, and rebroadcasts reset+hello+doc_update; refused while replay runs; the old session's row is closed first). Every result additionally carries `has_openai_key` and, when an undelivered objection exists, `pending_objection` `{id, kind, message, refs, status}` — including one fired during the current call (that is what `draft_add`'s ≤2s wait exists for; there is no separate `objection` field). Delivery watermark = the numeric objection-id suffix; only the newest undelivered objection is returned and the watermark advances past the gap.
...
- **REST endpoints** (thin, replay-style): `GET /state` → document snapshot `{title, blocks:[{id,text}], objections:[...], slug, has_openai_key}` via a new `session.snapshot(state)` helper; `POST /rpc` → body passed to `handle_client_message()` verbatim, returns `{ok:true}`. These back scripted testing.
- **VoiceOS stdio bridge:** `scripts/voiceos_mcp.py` (+ `scripts/voiceos_mcp.sh` launcher). VoiceOS Custom Integrations only spawn a local stdio command; the bridge forwards the same 8 tools to `http://127.0.0.1:8000/mcp` and returns spoken strings. No pipeline logic in the proxy.
...
- **Drafts history (in-app):** `GET /drafts` (list summary) and `GET /drafts/{slug}` (full content) read back from Convex through the server's Python client — no Convex JS in the main bundle. Router keywords `history`/`drafts` broadcast `{"type":"drafts_panel"}`; the client opens a read-only right-hand sidebar (DraftsPanel.jsx: unconfigured/error/empty/list/detail states, live-now tag for the current slug, `/drafts` added to the vite proxy).
